...
export default function Reception() {
	...
	const { data: receptions, setData: setReceptions } =
		useDynamicValues("reception");
	const dateToday = new Date();
	const todayReceptions = receptions.filter(
		(r) => formatDate(new Date(r.dateCreated)) == formatDate(dateToday)
	);
			...
			<div className="flex justify-content-between">
				...
				<span className="p-input-icon-left">
					...
					<Button
						label={
							isAll
								? "Visualizar Recepciones Actuales"
								: "Visualizar Todas las Recepciones"
						}
						icon={"pi pi-desktop"}
						onClick={() => setIsAll((prevData) => !prevData)}
					/>
				</span>
			</div>
		);
